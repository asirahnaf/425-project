"""Task 2 real-1000: train GNN vs CNN from prebuilt graphs+npz (1000 = 860 real + 140 aug).
Same protocol as train_task2_real.py, split 70/15/15 with seed 42.
Outputs:
  results/metrics_task2_real1000.json
  results/plots/task2_real1000_curves.png
  results/gnn_task2_real1000.pt + cnn_mel_1000.pt
"""
...
def main():
    ...
    ap.add_argument("--epochs_cnn", type=int, default=12)
    ap.add_argument("--tag", type=str, default="1000")
    args = ap.parse_args()
    ...
    c = Counter([it["track"].split(".")[0] for it in items])
    print("per-genre:", dict(c))
    random.Random(42).shuffle(items)
    n = len(items); ntr, nva = int(n*0.7), int(n*0.15)
    tr, va, te = items[:ntr], items[ntr:ntr+nva], items[ntr+nva:]
    print(f"split {len(tr)}/{len(va)}/{len(te)} device={device}")
    ...
    for ep in range(1, args.epochs_gnn+1):
        ...
        h_g.append(acc)
        if ep % 5 == 0 or ep == 1: print(f"GNN ep{ep}/{args.epochs_gnn} val={acc:.3f}")
    ...
    for ep in range(1, args.epochs_cnn+1):
        ...
        h_c.append(acc)
        print(f"CNN ep{ep}/{args.epochs_cnn} val={acc:.3f}")
    ...
    with torch.no_grad():
        ...
        cm_g = confusion_matrix(yt, pr, labels=list(range(10))).tolist()
        cm_c = confusion_matrix(tey.tolist(), pc, labels=list(range(10))).tolist()
    ...
    out = {
        "gnn": m_g, "cnn": m_c,
        "val_gnn": h_g, "val_cnn": h_c,
        "s_graph": coh,
        "n_total": n, "split": [len(tr), len(va), len(te)],
        "composition": "1000 = 860 real (storylinez) + 140 augmented pitch/time-stretch to reach 100/genre",
        "per_genre_total": dict(c),
        "confusion_gnn": cm_g, "confusion_cnn": cm_c,
        "epochs_gnn": args.epochs_gnn, "epochs_cnn": args.epochs_cnn,
        "genres": GENRES_10,
    }
    (res/f"metrics_task2_real{args.tag}.json").write_text(json.dumps(out, indent=2))
    fig, ax = plt.subplots()
    ax.plot(h_g, marker="o", label="GNN"); ax.plot(h_c, marker="s", label="CNN")
    ax.set_xlabel("epoch"); ax.set_ylabel("val acc"); ax.set_title(f"Task2 real{args.tag} GNN vs CNN")
    ax.legend(); fig.tight_layout()
    fig.savefig(res/"plots"/f"task2_real{args.tag}_curves.png", dpi=150)
    torch.save(gnn.state_dict(), res/f"gnn_task2_real{args.tag}.pt")
    torch.save(cnn.state_dict(), res/f"cnn_mel_{args.tag}.pt")
    print(f"saved real{args.tag} metrics + curves")
